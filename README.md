...
- `categories.js`: 물건 종별 분류(대분류 5 / 세분류 22)와 매각 형태. 서버와 브라우저가 같은 파일을 읽는다
...
대분류 5개 — 주거 · 상가·업무 · 산업 · 토지 · 기타. 세분류는 그 아래 22개다.
...
- 나머지는 `상가·오피스텔 미확정`으로 두고, `/api/officetel-price` 조회가 성공하면 확정한다.
  국세청 상업용건물·오피스텔 기준시가는 호실 단위로 용도(`O`/`S`)를 들고 있어서 PNU가 있으면 46%가 갈린다.
