...
// PROVINCE_EXTENTS 는 정식 명칭("부산광역시")을 쓰고 PROVINCE_CENTERS 는 줄임말("부산")도
// 섞여 있다. 이름이 정확히 같을 때만 찾으면 17개 중 8개(부산·대구·대전·울산·세종·강원·
// 전북·제주)가 조용히 빗나갔고, 그러면 아래에서 시·도 중심 대신 "화면에 걸친 사각형의
// 한가운데"를 찍어 마커가 지도를 움직일 때마다 따라 움직였다.
// 실측: 화면 오른쪽 끝을 129.40 → 129.15 로 줄이면 부산 마커 lng 가
// 129.1020 → 129.0660 으로 끌려왔다(경남은 이름이 맞아서 128.2132 에 고정).
function provinceCenterFor(name) {
  return (
    PROVINCE_CENTERS.find(([label]) => label === name) ||
    PROVINCE_CENTERS.find(([label]) => name.startsWith(label))
  );
}
...
export function provincesInBounds(bounds) {
  ...
  return PROVINCE_EXTENTS.map(([name, swLat, swLng, neLat, neLng]) => {
    ...
    const center = provinceCenterFor(name);
    ...
    return {
      name,
      // 마커를 겹치지 않게 밀더라도 자기 시·도 밖으로는 못 나가게 하려고 범위를 같이 준다.
      // 이게 없으면 최대 축소에서 서울 마커가 북한까지 밀려난다.
      extent: { swLat, swLng, neLat, neLng },
