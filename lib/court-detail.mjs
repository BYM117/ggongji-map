...
function normalizeDetail(raw) {
  ...
  return {
    ...
    documents,
    // 문서에서 뽑은 판단 재료. 파싱은 여기 한 곳에서만 한다 — 화면이 본문을 다시
    // 훑기 시작하면 같은 규칙이 두 곳에 생긴다.
    carryOver: findCarryOver(documents),
  ...
  return documents
    ...
    .map((doc) => {
      ...
      return {
        ...
        preview: readable ? documentPreview(doc.document_type, body) : ""
      };
    });
}
...
// 법원이 그 칸에 "없다"고 적는 방식들. 빈칸과 뜻이 같다.
const NO_CONTENT_MARK = /^(해당사항없음|해당없음|없음|-|없슴)$/;

// 칸을 읽어 세 갈래로 돌려준다.
//   { found: true,  carried: true  } 인수되는 권리가 적혀 있다
//   { found: true,  carried: false } 법원이 "해당사항없음"이라 적었거나 칸이 비어 있다
//   null                            그 칸이 본문에 아예 없다
//
// 셋째가 중요하다. 매각물건명세서가 별지(부동산의 표시)만 추출돼서 정작 1쪽이
// 통째로 없는 물건이 있다(2025타경14199, 883자). 그걸 "없음"으로 읽으면
// 인수 여부를 모르는 물건을 안전한 물건으로 보여주게 된다.
function documentHighlight(body) {
  ...
  for (const anchor of PREVIEW_ANCHORS) {
    ...
    // 칸이 비어 있으면 바로 다음 표제가 잡힌다. 표제를 내용처럼 보여주면 거짓말이다.
    if (!next || isDocumentHeading(next)) {
      return { label: anchor.label, value: anchor.empty, found: true, carried: false };
    }
    if (NO_CONTENT_MARK.test(next.replace(/\s+/g, ""))) {
      return { label: anchor.label, value: next, found: true, carried: false };
    }
    return { label: anchor.label, value: next, found: true, carried: true };
  }

  return null;
}

function documentPreview(type, body) {
  const highlight = documentHighlight(body);
  if (highlight) return `${highlight.label} · ${highlight.value}`.slice(0, 180);

  // 명세서인데 인수 칸이 본문에 없으면 별지만 추출된 것이다. 앞머리를 보여줘 봐야
  // "부동산의 표시 2025타경14199 …"라 쓸모가 없고, 무엇보다 그게 "인수 없음"으로
  // 읽히면 안 된다. 모르는 것은 모른다고 적는다.
  if (type === "매각물건명세서") return "인수 · 확인 불가 · 명세서 본문이 아니라 별지만 수집됐습니다";

  return body.slice(0, 180).replace(/\s+/g, " ").trim();
}

// 매각물건명세서가 "이건 낙찰자가 떠안는다"고 적어 놓은 것. 상세의 판정 배너가 쓴다.
// 현황조사서의 점유관계는 위험이 아니라 현황이라 여기 넣지 않는다.
//
// 경매 물건에는 공매와 달리 임대차 배열이 따로 오지 않는다. 그래서 이 값이 없으면
// "낙찰가에 얹히는 돈"이 판정 배너에 한 번도 나타나지 않는다 — 실측 2024타경56060은
// 최저입찰가 719만원에 보증금 4억 7천만원이 인수되는 물건인데 배너가 유찰 횟수만 말했다.
function findCarryOver(documents) {
  const doc = documents.find((item) => item.type === "매각물건명세서" && item.body);
  if (!doc) return null;
  const highlight = documentHighlight(doc.body);
  if (!highlight || highlight.label !== "인수") return null;
  return { state: highlight.carried ? "carried" : "none", text: highlight.value };
}
