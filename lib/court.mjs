...
// 화면이 넓으면 물건을 하나씩 내려보내는 것 자체가 안 된다. 전국은 32,000건이라
// 업스트림에서 받아오는 데만 26초가 걸려 Vercel 30초 제한에 걸려 죽었다(504).
// 그런데 업스트림은 "이 조건에 몇 건인가"는 1KB · 0.3초에 답해준다.
// 그래서 넓은 화면에서는 시·도별 개수만 물어보고 덩어리로 내려보낸다.
//
// 쪼개야 하는 시·도가 하나라도 있으면 시·군·구별 개수가 필요한데 그 표가 아직 없다.
// 그때는 null을 돌려 호출부가 기존 경로(물건 전체)로 가게 한다.
// (쪼갤지 말지의 기준은 app.js의 ADMIN_SPLIT_SHARE와 같다)
const CLUSTER_SPLIT_SHARE = 0.45;

export async function fetchCourtAuctionClusters(params, bounds) {
  const provinces = provincesInBounds(bounds);
  if (!provinces.length) return { ok: true, source: "법원경매", total: 0, clusters: [] };
  if (provinces.some((province) => province.share >= CLUSTER_SPLIT_SHARE)) return null;

  const counted = await mapWithConcurrency(provinces, 8, async (province) => {
    const countParams = new URLSearchParams(params);
    countParams.set("region", province.name);
    const result = await fetchJson(
      buildCourtAuctionUrl(countParams, { limit: 1, offset: 0, bounds }),
      courtAuctionRequestInit()
    );
    if (!result || result.error) return null;
    return { province, count: numberFrom(result.total) };
  });

  // 한 곳이라도 못 세면 총계가 틀린 지도를 보여주게 된다. 차라리 기존 경로로 넘긴다.
  if (counted.some((entry) => !entry)) return null;

  const clusters = counted
    .filter((entry) => entry.count > 0)
    .map(({ province, count }) => ({
      key: `admin:province:${province.name}`,
      label: province.name,
      lat: province.lat,
      lng: province.lng,
      extent: province.extent,
      count
    }))
  ...
  return {
    ...
    diagnostics: {
      source: "court",
      mode: "clusters",
      provincesInView: provinces.length,
      maxShare: Number(Math.max(...provinces.map((p) => p.share)).toFixed(3))
    }
  };
}
...
// 캐시 키는 "업스트림에 실제로 나가는 주소"에서 만든다. 직접 나열하지 않는 이유가 있다.
//
// 예전에는 키를 손으로 나열했는데, buildCourtAuctionUrl 의 passthrough 목록
// (q / region / status / source / sale_date_*)이 그 나열에 빠져 있었다.
// 그래서 필터가 걸린 결과가 필터 없는 키에 저장됐다. 실측: 캐시가 빈 상태에서
// "부산 경계 + q=없는검색어" 를 한 번 부르면 0건이 부산 키에 눌러앉아,
// 그 뒤 5분간 평범한 부산 요청도 전부 0건이 됐다(경계를 0.01도 옮기면 1,325건).
// 요청을 만드는 곳과 키를 만드는 곳이 둘이면 반드시 어긋난다. 하나로 묶는다.
// 뷰포트 행 스캔이 업스트림에 실어 보내도 되는 값은 화면 경계와 표시 조건뿐이다.
// 검색어·지역 같은 자유 문자열까지 통과시키면, 그 값이 캐시 키에 들어가 40칸짜리
// 행 캐시를 밖에서 마음대로 밀어낼 수 있다 — 실측: q 를 45번 바꿔 부른 뒤 평범한
// 서울 요청이 0.008초에서 1.60초로 돌아갔다(캐시가 전부 밀려났다).
// app.js 는 이 경로에 q 를 보내지 않고(보내는 건 경계·sources·exactGeocode·zoom 뿐),
// 전국 검색은 fetchCourtAuctionProperties 가 따로 맡는다. 여기서 받을 이유가 없다.
const VIEWPORT_SCAN_PARAMS = ["swLat", "swLng", "neLat", "neLng", "active", "require_coordinates", "sort"];

function viewportScanParams(params) {
  const scoped = new URLSearchParams();
  for (const key of VIEWPORT_SCAN_PARAMS) {
...
function courtAuctionRowsCacheKey(params, bounds, { scanLimit, maxScanPages }) {
  // 스냅샷 경로는 params 를 아예 보지 않고 파일만 읽는다. 나눌 것이 없다.
  if (!courtAuctionBaseUrl()) return "snapshot";

  const apiUrl = buildCourtAuctionUrl(params, { limit: scanLimit, offset: 0, bounds });
  // offset 은 한 키 안에서 여러 장을 이어 받는 값이라 캐시 단위가 아니다.
  apiUrl.searchParams.delete("offset");
  // 파라미터 순서가 달라도 같은 요청이면 같은 키여야 한다.
  apiUrl.searchParams.sort();
  // courtPages 는 업스트림 주소에 안 나가지만 몇 장까지 받을지를 바꾼다.
  return `${apiUrl.search}|pages=${maxScanPages}`;
}

async function getAllCourtAuctionRows(params) {
  // 아래로는 걸러낸 파라미터만 쓴다. 원본을 그대로 들고 다니면 언젠가 또 새어 나간다.
  const scanParams = viewportScanParams(params);
  const bounds = gridAlignedBounds(scanParams);
  ...
  const cacheKey = courtAuctionRowsCacheKey(scanParams, bounds, { scanLimit, maxScanPages });
  ...
  const task = (async () => {
    ...
    const fetchPage = async (pageIndex) => {
      const pageResult = await fetchJson(
        buildCourtAuctionUrl(scanParams, { limit: scanLimit, offset: pageIndex * scanLimit, bounds }),
        courtAuctionRequestInit()
      );
