...
async function getAllCourtAuctionRows(params) {
  ...
  const task = (async () => {
    ...
    for (let pageIndex = 0; pageIndex < maxScanPages; pageIndex += 1) {
      const pageOffset = pageIndex * scanLimit;
      const pageResult = await fetchJson(
        buildCourtAuctionUrl(params, { limit: scanLimit, offset: pageOffset, bounds }),
        courtAuctionRequestInit()
      );
      ...
      if (pageRows.length < scanLimit) {
        // 짧은 페이지는 "마지막 페이지"일 수도, 업스트림이 덜 준 것일 수도 있다.
        // total을 아는데 아직 못 채웠다면 후자다. 그걸 캐시하면 그 인스턴스는
        // 계속 잘린 목록만 보여주므로 불완전으로 남겨 다음 요청에서 다시 받는다.
        complete = !total;
        break;
      }
    }
...
function mapCourtAuctionRow(row) {
  ...
  const classifyAddress = pickClassifyAddress(
    ...
    row.address,
    address,
    normalizedAddress.detail ? `${cleanAddress} [${normalizedAddress.detail}]` : ""
  );
