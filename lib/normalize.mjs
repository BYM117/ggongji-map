...
import {
  ...
  groupIdOf,
  legacyType,
  needsStandardPriceLookup,
  refineSubWithBuildingPurpose,
  subLabel
} from "../categories.js";
...
export function normalizeProperty(raw) {
  ...
  return {
    ...
    buildingPurpose: String(raw.buildingPurpose || ""),
    ...
    // "유찰 0회"와 "유찰 횟수를 모른다"는 다른 값이다. || 0 으로 눌러 두면 둘이 똑같이
    // 0이 되고, 유찰 횟수를 아예 안 주는 공매 물건이 전부 "신건"으로 보인다.
    failCount: failCountFrom(raw),
...
// 값이 없으면 null(모름), 있으면 그 숫자. 0도 살려 둔다 — 법원 물건의 "유찰 0회"는
// 진짜 0회다(lib/court.mjs가 상태 문구에서 뽑아 실어 보낸다).
function failCountFrom(raw) {
  const value = firstValue(raw, ["failCount", "fail_count", "유찰횟수", "유찰수"]);
...
export async function resolveAmbiguousCategories(properties) {
  ...
  // 주용도만으로 좁힐 수 있는 물건도 있으므로 국세청 대상이 없다고 바로 나가지 않는다.
  if (!targets.length && !properties.some((item) => item?.categoryConfident === false && item.buildingPurpose)) {
    return properties;
  }
  ...
  // 국세청이 답을 못 낸 물건만 건축물대장 주용도로 좁힌다. 순서를 바꾸면 안 된다 —
  // 국세청은 호실 단위라 정확하고, 주용도는 필지 대표 건물 값이라 주상복합에서 어긋난다.
  for (const item of properties) {
    if (!item || resolved.has(item.id)) continue;
    if (item.categoryConfident !== false || !item.buildingPurpose) continue;
    const sub = refineSubWithBuildingPurpose(item.categorySub, item.buildingPurpose);
    if (sub !== item.categorySub) resolved.set(item.id, sub);
  }
