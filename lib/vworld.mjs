...
// 못 물어본 결과는 따로, 그리고 짧게만 들고 있는다. 아래 fetchGeocode 주석 참고.
const GEOCODE_FAILURE_TTL_MS = 60 * 1000;
const geocodeFailureCache = new Map(); // 주소 -> { payload, expiresAt }
...
export async function fetchGeocode(params) {
  ...
  const failed = geocodeFailureCache.get(address);
  if (failed && failed.expiresAt > Date.now()) return failed.payload;
  if (failed) geocodeFailureCache.delete(address);

  // fetchVworldPoint 는 "못 찾음"도 "못 물어봄"도 null 을 준다. 둘을 갈라 받는다.
  const status = {};
  const point = await fetchVworldPoint(address, status);
  const payload = point
    ? { ok: true, found: true, address, lat: point.lat, lng: point.lng, pnu: point.pnu || "", category: point.geocodeCategory }
    : { ok: true, found: false, address, upstreamFailed: Boolean(status.upstreamFailed) };

  // "그런 주소가 없다"와 "VWorld 에 못 물어봤다"를 같이 캐시하면, VWorld 가 회복돼도
  // 이 인스턴스는 그 주소를 영영 다시 묻지 않는다. 실측: 502 를 주는 동안 부른 주소가
  // found:false 로 굳었고, 같은 주소를 캐시 없이 직접 부르면 5번 다 정상으로 찾았다.
  //
  // 그렇다고 실패를 아예 안 들고 있으면 반대쪽으로 기운다 — 상대가 느린 동안 같은 주소를
  // 끝없이 다시 물어 상대를 더 느리게 만든다(느려짐 → 데드라인 초과 → 캐시 안 함 → 재요청).
  // 그래서 짧게만 들고 있는다. 회복은 1분 안에 반영되고, 장애 중 요청량은 그만큼 줄어든다.
  if (status.upstreamFailed) {
    putBoundedMap(geocodeFailureCache, address, { payload, expiresAt: Date.now() + GEOCODE_FAILURE_TTL_MS }, 5000);
  } else {
...
// 좌표 -> 행정구역(시도/시군구). 온비드 목록 API가 지역을 코드가 아니라 이름으로 받는데,
// 그 이름이 표준과 다른 게 섞여 있다(예: 전남광주통합특별시). VWorld 역지오코딩이
// 온비드와 같은 이름 체계를 돌려주므로 그대로 받아 쓴다. 직접 표를 들고 있으면
// 행정구역이 바뀔 때마다 어긋난다.
const regionCache = new Map();

// 배포 환경에서 VWorld 가 간헐적으로 끊긴다(이 저장소에 이미 기록이 있다).
// 왜 실패했는지 응답 진단에 실어 보내려고 마지막 사유만 들고 있는다.
let lastRegionError = "";
export function lastVworldRegionError() {
  return lastRegionError;
}

export async function fetchVworldRegion(lat, lng) {
  const key = process.env.VWORLD_API_KEY || process.env.PUBLIC_DATA_SERVICE_KEY;
  if (!key || !Number(lat) || !Number(lng)) return null;

  // 지도를 조금씩 움직일 때마다 같은 구를 다시 묻지 않도록 약 1km 격자로 반올림해 캐시한다.
  ...
  let region = null;
  try {
    // 지역 판정은 실패해도 대비 경로가 있다. 오래 매달리는 쪽이 훨씬 손해라 한 번만 짧게 묻는다.
    const payload = await fetchJsonRetry(apiUrl, undefined, { attempts: 1, timeoutMs: 2500 });
    const structure = payload?.response?.result?.[0]?.structure;
    if (structure?.level1) {
      region = { sido: String(structure.level1), sigungu: String(structure.level2 || "") };
    } else {
      lastRegionError = String(payload?.response?.status || payload?.response?.error?.text || payload?.text || "no_structure").slice(0, 80);
      return null;
    }
  } catch (error) {
    lastRegionError = String(error?.message || "fetch_failed").slice(0, 80);
    console.error("vworld reverse geocode failed", cacheKey, lastRegionError);
    return null; // 실패는 캐시하지 않는다. 다음 요청에서 다시 시도한다.
  }

  lastRegionError = "";

  putBoundedMap(regionCache, cacheKey, region, 5000);
  return region;
}

// status 를 넘기면 "못 물어봤다"를 status.upstreamFailed 로 알려준다.
// 모듈 전역 플래그를 쓰지 않는 이유는 하이드레이션이 한 번에 6~8건을 동시에 돌려서
// 서로의 실패 여부를 덮어쓰기 때문이다. 호출자마다 자기 객체를 들고 있어야 한다.
export async function fetchVworldPoint(address, status = {}) {
  const key = process.env.VWORLD_API_KEY || process.env.PUBLIC_DATA_SERVICE_KEY;
  if (!key || !address) return null;

  const queries = uniqueValues([address, simplifyAddressForSearch(address)]);
  const categories = ["PARCEL", "ROAD"];
  // 질의 2가지 x 분류 2가지를 차례로 시도한다. 상대가 느리면 이 조합만으로 한 주소에
  // 십수 초가 걸려, 여러 물건을 채우는 호출부가 함수 실행 제한에 걸린다.
  // 한 주소에 쓸 시간을 정해두고 넘기면 못 찾은 것으로 본다.
  const deadline = Date.now() + 4000;

  for (const query of queries) {
    for (const category of categories) {
      // 시간이 모자라 못 물어본 것도 "그런 주소가 없다"가 아니다.
      if (Date.now() > deadline) {
        status.upstreamFailed = true;
        return null;
      }
      const item = await fetchVworldAddressItem({ key, query, category }, status);
...
async function fetchVworldAddressItem({ key, query, category }, status) {
  ...
  try {
    // 주소 하나에 질의 2개 x 분류 2개가 돌아간다. 여기서 기본 3회 재시도까지 곱해지면
    // 주소 한 건에 최대 12번 요청이 나가고, 상대가 느릴 때 그 자체로 제한을 넘긴다.
    const result = await fetchJsonRetry(apiUrl, undefined, { attempts: 2, delayMs: 150, timeoutMs: 3000 });
    return result?.response?.result?.items?.[0] || null;
  } catch {
    // 여기서 삼키는 것은 맞다(다음 분류/질의를 계속 시도해야 한다). 다만 삼켰다는
    // 사실은 남겨야 호출부가 실패를 "확인 완료"로 캐시하지 않는다.
    if (status) status.upstreamFailed = true;
    return null;
  }
}
