...
> **⚠️ 2026-09-14부터 이 경로는 동작하지 않습니다.**
> `/api/properties`를 지우면서 `properties.json` · `onbid.csv` · `court-auction.csv`를
> 읽는 코드가 함께 사라졌습니다. 화면이 그 엔드포인트를 한 번도 부르지 않는데
> 인증 없이 열려 있어서, 누구나 서버의 API 키 할당량을 태울 수 있었기 때문입니다.
> 지금 지도에 뜨는 물건은 전부 `COURT_AUCTION_API_URL`(법원)과 온비드 OpenAPI에서 옵니다.
> 아래 설명은 되살릴 때를 위해 남겨 둡니다 — 커밋 `79cb1aa` 다음 커밋을 되돌리면 됩니다.
>
> 이 폴더에서 **지금도 쓰이는 것**은 `official-prices.json.gz`(사전 계산 공시기준가)와
> `officetel-prices/*.json.gz`(국세청 기준시가 인덱스)뿐입니다.
