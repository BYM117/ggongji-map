...
//
// **minBid는 2026-09-18에 lib/onbid.mjs에서 고쳤다.** 예전에는 목록의 minBid가 회차표의
// 마지막(아직 열리지도 않은 최저 회차) 가격으로 와서, 강서구 144건 중 122건이 "유찰 0회인데
// 감정가의 10%"로 떴다. 지금은 목록도 상세와 같은 회차 — 아직 안 끝난 회차 중 가장 이른 것 —
// 를 고르므로 아래 두 갈래는 같은 값을 낸다(실측 10건 전부 일치).
//
// 그래도 회차표를 먼저 보는 것을 남겨 둔다. 지역 목록을 끝까지 못 받은 화면
// (진단의 truncatedRegions > 0)에서는 목록이 이른 회차를 아예 못 봤을 수 있는데,
// 상세는 그 물건 하나만 따로 받아 오므로 회차표가 항상 온전하다.
// 여기서 minBid를 고쳐 쓰지는 않는다 — 그 값의 주인은 lib/onbid.mjs다.
